...
public class TuesdayListAdapter extends BaseAdapter {

	private ArrayList<WeekEvent> tuesdayList;
	private Context ctx;
	private LayoutInflater inflater;
	private DatabaseHandler db;
	private ImageProcessing imgProc;

	public TuesdayListAdapter(Context context, ArrayList<WeekEvent> tuesList) {
		tuesdayList = tuesList;
		ctx = context;
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		imgProc.setImageWith_loader(holder.ivAvatar, tuesdayList.get(position)
				.getEventLogo());
		
		final int eventId = tuesdayList.get(position).getEventId();
		final String eventTTS = tuesdayList.get(position).getEventTTS();
		final int selectMode = tuesdayList.get(position).get_select_mode();
		...
		holder.ivAvatar.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) 
			{
				...
				uela.selectionProcessWeekView(eventId, eventTTS, 0, position, 2);
			}
		});
		...
		holder.ivAvatar.setOnLongClickListener(new View.OnLongClickListener() {
			...
			{
				UserEventListActivity uela = (UserEventListActivity) ctx;
				uela.selectionProcessWeekView(eventId, eventTTS, 1,position, 2);
				return true;
			}
		});
