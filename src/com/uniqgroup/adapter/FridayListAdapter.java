...
public class FridayListAdapter extends BaseAdapter {

	private ArrayList<WeekEvent> fridayList;
	private Context ctx;
	private LayoutInflater inflater;
	private DatabaseHandler db;
	private ImageProcessing imgProc;

	public FridayListAdapter(Context context, ArrayList<WeekEvent> friList) {
		fridayList = friList;
		ctx = context;
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		imgProc.setImageWith_loader(holder.ivAvatar, fridayList.get(position)
				.getEventLogo());
		
		final int eventId = fridayList.get(position).getEventId();
		final String eventTTS = fridayList.get(position).getEventTTS();
		final int selectMode = fridayList.get(position).get_select_mode();
		...
		holder.ivAvatar.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) 
			{
				...
				uela.selectionProcessWeekView(eventId, eventTTS, 0, position, 5);
			}
		});
		...
		holder.ivAvatar.setOnLongClickListener(new View.OnLongClickListener() {
			...
			{
				UserEventListActivity uela = (UserEventListActivity) ctx;
				uela.selectionProcessWeekView(eventId, eventTTS, 1,position, 5);
				return true;
			}
		});
		return convertView;
