...
@SuppressLint("NewApi")
public class Custom_cell_daywork2 extends RelativeLayout {
	...
	public Custom_cell_daywork2(Context context, Bitmap picture,
	...
	public Custom_cell_daywork2(Context context, AttributeSet attrs) {
		super(context, attrs);
		// timerTime=Time;
		init(context);
		ctx = context;
	...
	public void init(final Context ctx) {
		...
		View v = inflater.inflate(R.layout.custom_cell_daywork2, null);

		/** control the states with these layouts **/
		main_view_container = (LinearLayout) v.findViewById(R.id.container_linear_cell2);
		stick_hanger_view = (View) v.findViewById(R.id.stick_hanger_cell2);
		roundHanger_tv = (TextView) v.findViewById(R.id.round_hanger_cell2);

		status_textV = (TextView) v.findViewById(R.id.status_customcell_textV2);
		work_pic_IV = (ImageView) v
				.findViewById(R.id.workpic_customcellDaywork_IV2);
		if (bitmapToSet != null)
			work_pic_IV.setImageBitmap(bitmapToSet);

		tick_pic_IV = (ImageView) v
				.findViewById(R.id.tickpicc_customcellDaywork_IV2);
		//controlling the visibility of textview and action IV
		if (events != null) {

			if (events.getStatus().equals("0") || events.getStatus() == null) {
				...
				if (tts_text != null)
					...
					|| events.getStatus().equals("2")) {
				status_textV.setVisibility(View.INVISIBLE);
				if(events.getStatus().equals("1")){
					setState(1);
				}
				if(events.getStatus().equals("2")){
					setState(2);
				}
			}
